/* KDHeap.c: Mehrdimensionale Prioritaetswarteschlangen
 * 
...
#if 0
#define GREATER(h,d,x,y)  (EL_GREATER(d,*x,*y))
#define IS_INFTY(d,x) (EL_IS_INFTY(d,x))
#else
#define GREATER(h,d,x,y) (h->fcts.greater(d,x,y,h->fcts.greaterContext))
#define IS_INFTY(d,x) (h->fcts.isInfty(d,x))
#endif
...
static unsigned int firstIndex(KDHeapT h)
{
  return (h->k > MAGIC_K) ? 0 : 1;
}
static unsigned int lastIndex(KDHeapT h)
{
  return (h->k > MAGIC_K) ? h->i - 1 : h->i;
}

static void setObserver(ElementT *x, unsigned long index)
{
  if (x->observer != NULL){
    *(x->observer) = index;
  }
...
static void swap(ElementT *x, ElementT *y)
{
  ElementT t = *x;
  *x = *y;
  *y = t;
...
static void decreaseI(KDHeapT h)
{
...
static void extendHeap(KDHeapT h)
/* Vergroessert den Heap */
{
  h->e  = ENLARGEMENT(h->e);
  h->xs = our_realloc(h->xs, h->e * sizeof(ElementT), "Extending KDHeap");
}

#if TESTHEAP == 2
static void printHeap(KDHeapT h)
{
  unsigned long i;
...
static void checkNode(KDHeapT h, unsigned long index)
...
static void checkNode2(KDHeapT h, unsigned long index)
/* Checkt die Heap-Bedingung fuer ein Element 
...
static void checkHeap(KDHeapT h)
/* Checkt die Heap-Bedingung fuer alle Elemente */
{
  ...
  if (h->k > MAGIC_K){
    ...
    if (level(h->i) != h->level_i){
      ...
      our_error("CheckHeap: Violated KDHeap invariant!");
    }
    for (i = 1; i <= h->i; i++){
      checkNode(h,i);
    }
...
static void find_min(KDHeapT h, unsigned long index, 
...
static void normalize_up(KDHeapT h, unsigned long index, 
...
static void normalize_down(KDHeapT h, unsigned long index, 
...
/* sons (i) | (i+1)%k == 0  = {2(i+1)-k,2(i+1)}
            | otherwise     = {i+1}
*/ 
static unsigned long block(KDHeapT h, unsigned long i)
{
  return i / h->k + 1;
}

static unsigned slot (KDHeapT h, unsigned long i)
{
  return i % h->k;
}

static void find_min2(KDHeapT h, 
...
static void normalize_up2(KDHeapT h, 
...
static void normalize_up2Speziale(KDHeapT h, 
...
static void normalize_down2(KDHeapT h, unsigned long index, unsigned sindex)
/* fuer vieldimensionalen Heap (k > MAGIC_K) */
{
  unsigned long min;
  unsigned smin;
...
static void upheap(KDHeapT h, unsigned long hole, ElementT x)
...
static void downheap(KDHeapT h, unsigned long hole, ElementT x)
...
static void deleteElement(KDHeapT h, ElementT *res, 
...
static void ReorgHeap(KDHeapT h)
{
  unsigned long i;
  if (h->k > MAGIC_K){
    for (i = h->i; i >= 1; i--){
...
{
  KDHeapT res = our_alloc(sizeof(*res));
  if (!((1 <= k) && (k <= 50))){
    our_error("KDH_mkHeap: k out of sensible bounds!");
  }
  res->k       = k;
  res->i       = 0;
  res->level_i = 0;
  ...
  res->xs = our_alloc(res->e * sizeof(ElementT));
...
BOOLEAN KDH_getMin(KDHeapT h, ElementT *res, unsigned d)
{
  ...
  HEAPTEST(h);
  if (!(1 <= h->i)){
    our_error("KDH_getMin: Delete on empty heap!");
  }
  if (d >= h->k){
    our_error("KDH_getMin: Given dimension to large!");
  }
...
BOOLEAN KDH_deleteMin(KDHeapT h, ElementT *res, unsigned d)
{
  ...
  HEAPTEST(h);
  if (!(1 <= h->i)){
    our_error("KDH_deleteMin: Delete on empty heap!");
  }
  if (d >= h->k){
    our_error("KDH_deleteMin: Given dimension to large!");
  }
  /* in den obersten d+1 Ebenen suchen */
  if (h->k > MAGIC_K){
    find_min2(h,0,0,d,d,&min,&min_level);
...
void KDH_deleteElement(KDHeapT h, unsigned long index, ElementT *res)
{
  HEAPTEST(h);
  if (!(index < h->i)){
    our_error("KDH_deleteElement: Delete element with index out of range!");
  }
  if (h->k > MAGIC_K){
 ...
 */
{
  HEAPTEST(h);
  if (!(index < h->i)){
    our_error("KDH_getElement: Element with index out of range!");
  }
  if (h->k > MAGIC_K){
    return &(h->xs[index]);
  }
...
{
  if (!(index < h->i)){
    our_error("KDH_heapifyElement: Heapify element with index out of range!");
  }
  if (h->k > MAGIC_K){           /* vieldimensionaler Heap */
...
void KDH_ToScreen(KDHeapT h) {
  ...
  while (index <= lastIndex(h)) {
    ...
    for (i = 0; (i < levelWidth) && (index <= lastIndex(h)); ++i) {
      ...
      printf("        ");
      printf(" <%ld, %lu, %lu, %lu>\n", h->xs[index].w1, h->xs[index].t1, h->xs[index].t2, h->xs[index].xp);
      index++;
    }
    level++;
...
int main (int argn, char* argv[])
{
  ...
  printf ("Dimension: %d\n", dim);
  h = KDH_mkHeap(10000, dim, greater);
...
#if 0
/opt/utils/shadekit/shade.v9.elf/bin/spixcounts -c KDHeap
/opt/utils/shadekit/spixtools.v9.elf/bin/sprint -b KDHeap.1.bb KDHeap KDHeap.c | p
#endif

...
static unsigned int  ct41_level = 0;
static unsigned int  ct41_tree_counter = 0;
static unsigned long ct41_node_counter = 0;
...
static HintT ct41_mkHint(unsigned long h, unsigned long pos, unsigned no)
{
...
static BOOLEAN ct41_doLeaf(HintT h)
{
  //  return 1;
  return h & LeafHint;
}

static BOOLEAN ct41_doInst(HintT h)
{
  //  return 1;
  return h & InstHint;
}

static BOOLEAN ct41_doSplit(HintT h)
{
  //  return 1;
  return h & SplitHint;
}

static BOOLEAN ct41_doRewr(HintT h)
{
  //  return 1;
  return h & RewrHint;
}

static BOOLEAN ct41_doPath(HintT h)
{
  //  return 0;
  return h & PathHint;
}

static BOOLEAN ct41_doCRPath(HintT h)
{
  //  return 0;
  return h & CRPathHint;
}

static unsigned long ct41_getPos(HintT h)
{
  //  return 0;
...
static unsigned long ct41_getNo(HintT h)
{
  //  return 0;
...
static BOOLEAN ct41_handleNode (LiteralT l, ContextT c, EConsT e, HintT h);

/* -------------------------------------------------------------------------- */
/* - generische Hilfsroutingen ---------------------------------------------- */
/* -------------------------------------------------------------------------- */

static void ct41_print_DConsKetten(DConsT d,DConsT d1)
{
...
static BOOLEAN ct41_equalUptoPos(LiteralT l1, UTermT p, LiteralT l2)
{
...
static void ct41_Durchnumerieren(UTermT t)
{
...
static BOOLEAN ct41_LeafTests(LiteralT l, 
                             ...
                             BOOLEAN *closed)
{
  ...
  OPENTRACE (("ct41_LeafTests: %t %C %E\n", l, c, e));
  {
    TermT s = TO_LitLHS(l);
    TermT t = TO_LitRHS(l);
    if (TO_TermeGleich(s,t)){
      ...
      if (TEX_OUTPUT) {
        IO_DruckeFlex("%*\\Subsumed{%d}%%\n", 
                      2*ct41_level, RE_AbsNr(MO_AngewendetesObjekt));
      }
      handled = *closed = TRUE;
    }
  ...
  CLOSETRACE (("ct41_LeafTests: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct41_Split(LiteralT l, ContextT c, EConsT e,
                         HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct41_Split: %t %C %E\n", l, c, e));
  {
    if (e != NULL){
      ...
      if (d == NULL){ /* OK, vorhin wurde andere Seite bei CRewrite */
        *closed = TRUE;   /* als erfuellbar nachgewiesen. */
        ...
        HintT h1 = ct41_mkHint(InstHint|RewrHint,lambdaPos,0);
        DConsT drun;
        DConsT d1 = d;
        BOOLEAN allclosed = TRUE;
        if (PA_optDCons()){
          d1 = CO_minimizeDCons(d);
          ct41_print_DConsKetten(d,d1);
        }
        if (PA_sortDCons()){
          CO_sortDCons(d1);
        }
        ...
        do {
          allclosed = ct41_handleNode(l,drun->c,NULL,h1);
          drun = drun->Nachf;
  ...
  CLOSETRACE (("ct41_Split: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct41_Inst(LiteralT l, ContextT c, EConsT e,
                        HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct41_Inst: %t %C %E\n", l, c, e));
  {
    if (CO_somethingBound(c->bindings)){
      HintT h1;
      unsigned int pos = ct41_getPos(h);
      LiteralT l1 = CO_substitute(c->bindings,l);
      ContextT c1 = CO_solveEQPart(c);
      if (e != NULL){
        our_error("rethink for e != NULL!");
      }
      ct41_Durchnumerieren(l1);
      if (TEX_OUTPUT) IO_DruckeFlex("%*\\Inst{%%\n", 2*ct41_level);
      ct41_level++;
      h1 = ct41_mkHint(LeafHint|RewrHint,lambdaPos,0);
      *closed = ct41_handleNode(l1,c1,NULL,h1);
      CO_deepDelete(c1);
      TO_TermLoeschen(l1);
      handled = TRUE;
    }
  }
  CLOSETRACE (("ct41_Inst: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct41_XRew (LiteralT l, ContextT c, EConsT e,
                         HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct41_XRew: %t  %C %E\n", l, c, e));
  {
    GleichungsT gleichung;
    unsigned int ppos   = ct41_getPos(h);
    unsigned int endNum = 0;
    unsigned int pos    = ct41_doPath(h) ? 1 : ppos;
    unsigned int no     = ct41_doPath(h) ? 0 : ct41_getNo(h);
    unsigned int no2;
    UTermT l1           = TO_TermAnStelle(l,pos);
    UTermT l0           = TO_NULLTerm(l);
    if (ct41_doCRPath(h) && ct41_doPath(h) && (ppos != lambdaPos)){
    ...
    do {
      if ((endNum != 0) /*&& (pos < ppos)*/ && (TO_GetId(TO_TermEnde(l1)) < endNum)){ 
        if(0)IO_DruckeFlex("Skip position %P (%t) because %d < %d last rewrite at %t (%d) = %P\n",
                      ...
                      TO_TermAnStelle(l,ct41_getPos(h)), TO_GetId(TO_TermAnStelle(l,ct41_getPos(h))),
                      l, TO_TermAnStelle(l,ct41_getPos(h)));
        if (0){
          l1 = TO_Schwanz(l1);
          pos++;
        }
        ...
        if (TO_TermIstNichtVar(l1)){
          if (no == 0 && 
            ...
            HintT h1 = ct41_mkHint(LeafHint|RewrHint|PathHint,pos,0);
            handled = TRUE;
            ct41_Durchnumerieren(lneu);
            if (TEX_OUTPUT) {
            ...
            ct41_level++;
            *closed = ct41_handleNode(lneu,c,e,h1);
            ...
            while (!handled && 
              ...
              if (no2 <= no){
                ...
                if (CO_satisfiableUnder(e1,c)){
                  ...
                  HintT h1 = ct41_mkHint(LeafHint|RewrHint,lambdaPos,0);
                  HintT h2 = ct41_mkHint(CrewHint,pos,no2);
                  handled = TRUE;
                  if (TEX_OUTPUT){
                  ...
                  ct41_level++;
                  *closed = PA_preferD1()
                    ? ct41_handleNode(lneu,c,e1,h1) && ct41_handleNode(l,c,e2,h2)
                    : ct41_handleNode(l,c,e2,h2) && ct41_handleNode(lneu,c,e1,h1);
                  CO_deleteEcons(e2);
                }
                else {
                  if (TEX_OUTPUT_F){
                    IO_DruckeFlex("%*\\FCRewrite{%d%s}{%P}{%E \\wedge %C}%%\n", 
                                  2*ct41_level, RE_AbsNr(gleichung), 
                                  TP_RichtungAusgezeichnet(gleichung) ? "" :"'",
                                  l, l1, e1, c);
                  }
                }
  ...
  CLOSETRACE (("ct41_XRew -> %b\n", handled));
  return handled;
}

/* Format von ct41_handleNode
 * static 
...
static BOOLEAN ct41_handleNode (LiteralT l, ContextT c, EConsT e, HintT h)
{
  ...
  OPENTRACE (("ct41_handleNode: %t %C %E\n", l, c, e));
  {
    unsigned int oldlevel = ct41_level;
    ct41_node_counter++;
    if (TEX_OUTPUT) {
      IO_DruckeFlex("%*\\Node{%t}{%t}{%E%C}{%%\n", 
                    2*ct41_level, TO_LitLHS(l), TO_LitRHS(l), e, c);
    }
    ct41_level++;
    if (LIMIT_DEPTH && (ct41_level > DEPTH_LIMIT)){
      /* to catch endless recursion */
      our_error("ct41_handleNode: nesting to deep");
    }
    else if ((PA_maxNodeCount() != 0) && 
             (ct41_node_counter >= PA_maxNodeCount())){
      if (TEX_OUTPUT) IO_DruckeFlex("%*\\ToManyNodes%%\n", 2*ct41_level);
      closed = FALSE;
    }
    else if (ct41_doLeaf(h) && ct41_LeafTests(l,c,e,h,&closed)){
    }
    else if (ct41_doInst(h)  && ct41_Inst(l,c,e,h,&closed)){
    }
    else if (ct41_Split(l,c,e,h,&closed)){
    }
    else if (ct41_XRew(l,c,e,h,&closed)){
    }
    else {
      if (TEX_OUTPUT) IO_DruckeFlex("%*\\GiveUp%%\n", 2*ct41_level);
      closed = FALSE; /* Machen wir's nochmals explizit */
    }
    while (ct41_level > oldlevel){
      ct41_level--;
      if (TEX_OUTPUT) IO_DruckeFlex("%*}%%\n", 2*ct41_level);
    }
  }
  CLOSETRACE (("ct41_handleNode: -> %b\n", closed));
  return closed;
}
...
BOOLEAN CO_GroundJoinable41(TermT s, TermT t)
{
  BOOLEAN res = FALSE;
  {
    ...
    LiteralT l;
    ContextT c;
    HintT h = ct41_mkHint(CrewHint,lambdaPos,0);
    ss = TO_Termkopie(s);
    tt = TO_Termkopie(t);
    TO_mkLit(l,ss,tt);
    ct41_Durchnumerieren(l);
    c = CO_newContext(CO_allocBindings());
    ct41_tree_counter++;
    ct41_node_counter = 0;
    if(TEX_OUTPUT) IO_DruckeFlex("\\ctree{%d}{%%\n", ct41_tree_counter);
    if (ct41_tree_counter == 50){
      printf("Hallo\n");
    }
    ct41_level++;
    res = ct41_handleNode(l,c,NULL,h);
    if(0)IO_DruckeFlex("#%l %b\n", ct41_node_counter, res);

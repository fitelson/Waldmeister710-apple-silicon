...
static unsigned int  ct62_level = 0;
static unsigned int  ct62_tree_counter = 0;
static unsigned long ct62_node_counter = 0;
...
static HintT ct62_mkHint(unsigned long h, unsigned long pos, unsigned no)
{
...
static BOOLEAN ct62_doLeaf(HintT h)
{
  //  return 1;
  return h & LeafHint;
}

static BOOLEAN ct62_doInst(HintT h)
{
  //  return 1;
  return h & InstHint;
}

static BOOLEAN ct62_doSplit(HintT h)
{
  //  return 1;
  return h & SplitHint;
}

static BOOLEAN ct62_doRewr(HintT h)
{
  //  return 1;
  return h & RewrHint;
}

static BOOLEAN ct62_doPath(HintT h)
{
  //  return 0;
  return h & PathHint;
}

static BOOLEAN ct62_doCRPath(HintT h)
{
  //  return 0;
  return h & CRPathHint;
}

static unsigned long ct62_getPos(HintT h)
{
  //  return 0;
...
static unsigned long ct62_getNo(HintT h)
{
  //  return 0;
...
static BOOLEAN ct62_handleNode (LiteralT l, ContextT c, EConsT e, HintT h);

/* -------------------------------------------------------------------------- */
/* - generische Hilfsroutingen ---------------------------------------------- */
/* -------------------------------------------------------------------------- */

static void ct62_print_DConsKetten(DConsT d,DConsT d1)
{
...
static BOOLEAN ct62_equalUptoPos(LiteralT l1, UTermT p, LiteralT l2)
{
...
static void ct62_Durchnumerieren(UTermT t)
{
...
static BOOLEAN ct62_LeafTests(LiteralT l, 
                             ...
                             BOOLEAN *closed)
{
  ...
  OPENTRACE (("ct62_LeafTests: %t %C %E\n", l, c, e));
  {
    TermT s = TO_LitLHS(l);
    TermT t = TO_LitRHS(l);
    if (TO_TermeGleich(s,t)){
      ...
      if (TEX_OUTPUT) {
        IO_DruckeFlex("%*\\Subsumed{%d}%%\n", 
                      2*ct62_level, RE_AbsNr(MO_AngewendetesObjekt));
      }
      handled = *closed = TRUE;
    }
  ...
  CLOSETRACE (("ct62_LeafTests: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct62_Split(LiteralT l, ContextT c, EConsT e,
                         HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct62_Split: %t %C %E\n", l, c, e));
  {
    if (e != NULL){
      ...
      if (d == NULL){ /* OK, vorhin wurde andere Seite bei CRewrite */
        *closed = TRUE;   /* als erfuellbar nachgewiesen. */
        ...
        HintT h1 = ct62_mkHint(InstHint|RewrHint|ct62_doPath(h)|ct62_doCRPath(h),
                              ct62_getPos(h),ct62_getNo(h));
        DConsT drun;
        DConsT d1 = d;
        BOOLEAN allclosed = TRUE;
        if (PA_optDCons()){
          d1 = CO_minimizeDCons(d);
          ct62_print_DConsKetten(d,d1);
        }
        if (PA_sortDCons()){
          CO_sortDCons(d1);
        }
        ...
        do {
          allclosed = ct62_handleNode(l,drun->c,NULL,h1);
          drun = drun->Nachf;
  ...
  CLOSETRACE (("ct62_Split: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct62_Inst(LiteralT l, ContextT c, EConsT e,
                        HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct62_Inst: %t %C %E\n", l, c, e));
  {
    if (CO_somethingBound(c->bindings)){
      HintT h1;
      unsigned int pos = ct62_getPos(h);
      LiteralT l1 = CO_substitute(c->bindings,l);
      ContextT c1 = CO_solveEQPart(c);
      if (e != NULL){
        our_error("rethink for e != NULL!");
      }
      ct62_Durchnumerieren(l1);
      if (TEX_OUTPUT) IO_DruckeFlex("%*\\Inst{%%\n", 2*ct62_level);
      ct62_level++;
      if (ct62_equalUptoPos(l,TO_TermAnStelle(l,pos),l1)){
        if(0)IO_DruckeFlex("EQUAL UP TO POS: %t %t (%P == %d == %t)\n",
                      l, l1, l, TO_TermAnStelle(l,ct62_getPos(h)), 
                      ct62_getPos(h), TO_TermAnStelle(l,ct62_getPos(h)));
        h1 = ct62_mkHint(LeafHint|RewrHint|PathHint|ct62_doSplit(h),
                        ct62_getPos(h),ct62_getNo(h));
      }
      else {
        h1 = ct62_mkHint(LeafHint|RewrHint|ct62_doSplit(h),lambdaPos,0);
      }
      *closed = ct62_handleNode(l1,c1,NULL,h1);
      CO_deepDelete(c1);
      TO_TermLoeschen(l1);
      handled = TRUE;
    }
  }
  CLOSETRACE (("ct62_Inst: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct62_Rewrite(LiteralT l, ContextT c, EConsT e,
                           HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct62_Rewrite: %t %C %E\n", l, c, e));
  {
    unsigned int ppos   = ct62_getPos(h);
    unsigned int endNum = 0;
    unsigned int pos    = ct62_doPath(h) ? 1 : ppos;
    ...
    if (ct62_doPath(h) && (ppos != lambdaPos)){
    ...
    do {
      if ((endNum != 0) /*&& (pos < ppos)*/ && (TO_GetId(TO_TermEnde(l1)) < endNum)){ 
        if(0)IO_DruckeFlex("Skip position %P (%t) because %d < %d last rewrite at %t (%d) = %P\n",
                      ...
                      TO_TermAnStelle(l,ct62_getPos(h)), TO_GetId(TO_TermAnStelle(l,ct62_getPos(h))),
                      l, TO_TermAnStelle(l,ct62_getPos(h)));
        if (0){
          l1 = TO_Schwanz(l1);
          pos++;
        }
        ...
        if (TO_TermIstNichtVar(l1) &&
          ...
          HintT h1 = ct62_mkHint(LeafHint|RewrHint|PathHint|ct62_doSplit(h),
                                pos,0);
          handled = TRUE;
          ct62_Durchnumerieren(lneu);
          if (TEX_OUTPUT) {
          ...
          ct62_level++;
          *closed = ct62_handleNode(lneu,c,e,h1);
  ...
  CLOSETRACE (("ct62_Rewrite: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct62_CRew (LiteralT l, ContextT c, EConsT e,
                         HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct62_CRew: %t  %C %E\n", l, c, e));
  {
    GleichungsT gleichung;
    unsigned int ppos   = ct62_getPos(h);
    unsigned int endNum = 0;
    unsigned int pos    = ct62_doPath(h) ? 1 : ppos;
    unsigned int no     = ct62_doPath(h) ? 0 : ct62_getNo(h);
    unsigned int no2;
    UTermT l1           = TO_TermAnStelle(l,pos);
    UTermT l0           = TO_NULLTerm(l);
    if (ct62_doCRPath(h) && ct62_doPath(h) && (ppos != lambdaPos)){
      endNum = TO_GetId(TO_TermEnde(TO_TermAnStelle(l,ppos)));
    }
    do {
      if ((endNum != 0) /*&& (pos < ppos)*/ && (TO_GetId(TO_TermEnde(l1)) < endNum)){ 
          ...
          while (!handled && 
            ...
            if (no2 <= no){
              ...
              if (CO_satisfiableUnder(e1,c)){
                ...
                HintT h1 = ct62_mkHint(LeafHint|PathHint|CRPathHint|SplitHint,pos,0);
                HintT h2 = ct62_mkHint(CrewHint|SplitHint,pos,no2);
                handled = TRUE;
                if (TEX_OUTPUT){
                ...
                ct62_level++;
                *closed = PA_preferD1()
                  ? ct62_handleNode(lneu,c,e1,h1) && ct62_handleNode(l,c,e2,h2)
                  : ct62_handleNode(l,c,e2,h2) && ct62_handleNode(lneu,c,e1,h1);
                CO_deleteEcons(e2);
              }
              else {
                if (TEX_OUTPUT_F){
                  IO_DruckeFlex("%*\\FCRewrite{%d%s}{%P}{%E \\wedge %C}%%\n", 
                                2*ct62_level, RE_AbsNr(gleichung), 
                                TP_RichtungAusgezeichnet(gleichung) ? "" :"'",
                                l, l1, e1, c);
                }
              }
  ...
  CLOSETRACE (("ct62_CRew -> %b\n", handled));
  return handled;
}

/* Format von ct62_handleNode
 * static 
...
static BOOLEAN ct62_handleNode (LiteralT l, ContextT c, EConsT e, HintT h)
{
  ...
  OPENTRACE (("ct62_handleNode: %t %C %E\n", l, c, e));
  {
    unsigned int oldlevel = ct62_level;
    ct62_node_counter++;
    if (TEX_OUTPUT) {
      IO_DruckeFlex("%*\\Node{%t}{%t}{%E%C}{%%\n", 
                    2*ct62_level, TO_LitLHS(l), TO_LitRHS(l), e, c);
    }
    ct62_level++;
    if (LIMIT_DEPTH && (ct62_level > DEPTH_LIMIT)){
      /* to catch endless recursion */
      our_error("ct62_handleNode: nesting to deep");
    }
    else if ((PA_maxNodeCount() != 0) && 
             (ct62_node_counter >= PA_maxNodeCount())){
      if (TEX_OUTPUT) IO_DruckeFlex("%*\\ToManyNodes%%\n", 2*ct62_level);
      closed = FALSE;
    }
    else if (ct62_doLeaf(h) && ct62_LeafTests(l,c,e,h,&closed)){
    }
    else if (ct62_doInst(h)  && ct62_Inst(l,c,e,h,&closed)){
    }
    else if (ct62_doSplit(h) && ct62_Split(l,c,e,h,&closed)){
    }
    else if (ct62_doRewr(h)  && ct62_Rewrite(l,c,e,h,&closed)){
    }
    else if (ct62_CRew(l,c,e,h,&closed)){
    }
    else {
      if (TEX_OUTPUT) IO_DruckeFlex("%*\\GiveUp%%\n", 2*ct62_level);
      closed = FALSE; /* Machen wir's nochmals explizit */
    }
    while (ct62_level > oldlevel){
      ct62_level--;
      if (TEX_OUTPUT) IO_DruckeFlex("%*}%%\n", 2*ct62_level);
    }
  }
  CLOSETRACE (("ct62_handleNode: -> %b\n", closed));
  return closed;
}
...
BOOLEAN CO_GroundJoinable62(TermT s, TermT t)
{
  BOOLEAN res = FALSE;
  {
    ...
    LiteralT l;
    ContextT c;
    HintT h = ct62_mkHint(CrewHint,lambdaPos,0);
    ss = TO_Termkopie(s);
    tt = TO_Termkopie(t);
    TO_mkLit(l,ss,tt);
    ct62_Durchnumerieren(l);
    c = CO_newContext(CO_allocBindings());
    ct62_tree_counter++;
    ct62_node_counter = 0;
    if(TEX_OUTPUT) IO_DruckeFlex("\\ctree{%d}{%%\n", ct62_tree_counter);
    ct62_level++;
    res = ct62_handleNode(l,c,NULL,h);
    if(0)IO_DruckeFlex("#%l %b\n", ct62_node_counter, res);

...
static unsigned int  ct42_level = 0;
static unsigned int  ct42_tree_counter = 0;
static unsigned long ct42_node_counter = 0;
...
static HintT ct42_mkHint(unsigned long h, unsigned long pos, unsigned no)
{
...
static BOOLEAN ct42_doLeaf(HintT h)
{
  //  return 1;
  return h & LeafHint;
}

static BOOLEAN ct42_doInst(HintT h)
{
  //  return 1;
  return h & InstHint;
}

static BOOLEAN ct42_doSplit(HintT h)
{
  //  return 1;
  return h & SplitHint;
}

static BOOLEAN ct42_doRewr(HintT h)
{
  //  return 1;
  return h & RewrHint;
}

static BOOLEAN ct42_doPath(HintT h)
{
  //  return 0;
  return h & PathHint;
}

static BOOLEAN ct42_doCRPath(HintT h)
{
  //  return 0;
  return h & CRPathHint;
}

static unsigned long ct42_getPos(HintT h)
{
  //  return 0;
...
static unsigned long ct42_getNo(HintT h)
{
  //  return 0;
...
static BOOLEAN ct42_handleNode (LiteralT l, ContextT c, EConsT e, HintT h);

/* -------------------------------------------------------------------------- */
/* - generische Hilfsroutingen ---------------------------------------------- */
/* -------------------------------------------------------------------------- */

static void ct42_print_DConsKetten(DConsT d,DConsT d1)
{
...
static BOOLEAN ct42_equalUptoPos(LiteralT l1, UTermT p, LiteralT l2)
{
...
static void ct42_Durchnumerieren(UTermT t)
{
...
static BOOLEAN ct42_LeafTests(LiteralT l, 
                             ...
                             BOOLEAN *closed)
{
  ...
  OPENTRACE (("ct42_LeafTests: %t %C %E\n", l, c, e));
  {
    TermT s = TO_LitLHS(l);
    TermT t = TO_LitRHS(l);
    if (TO_TermeGleich(s,t)){
      ...
      if (TEX_OUTPUT) {
        IO_DruckeFlex("%*\\Subsumed{%d}%%\n", 
                      2*ct42_level, RE_AbsNr(MO_AngewendetesObjekt));
      }
      handled = *closed = TRUE;
    }
  ...
  CLOSETRACE (("ct42_LeafTests: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct42_Split(LiteralT l, ContextT c, EConsT e,
                         HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct42_Split: %t %C %E\n", l, c, e));
  {
    if (e != NULL){
      ...
      if (d == NULL){ /* OK, vorhin wurde andere Seite bei CRewrite */
        *closed = TRUE;   /* als erfuellbar nachgewiesen. */
        ...
        HintT h1 = ct42_mkHint(InstHint|RewrHint|ct42_doPath(h)|ct42_doCRPath(h),
                              ct42_getPos(h),ct42_getNo(h));
        DConsT drun;
        DConsT d1 = d;
        BOOLEAN allclosed = TRUE;
        if (PA_optDCons()){
          d1 = CO_minimizeDCons(d);
          ct42_print_DConsKetten(d,d1);
        }
        if (PA_sortDCons()){
          CO_sortDCons(d1);
        }
        ...
        do {
          allclosed = ct42_handleNode(l,drun->c,NULL,h1);
          drun = drun->Nachf;
  ...
  CLOSETRACE (("ct42_Split: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct42_Inst(LiteralT l, ContextT c, EConsT e,
                        HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct42_Inst: %t %C %E\n", l, c, e));
  {
    if (CO_somethingBound(c->bindings)){
      HintT h1;
      unsigned int pos = ct42_getPos(h);
      LiteralT l1 = CO_substitute(c->bindings,l);
      ContextT c1 = CO_solveEQPart(c);
      if (e != NULL){
        our_error("rethink for e != NULL!");
      }
      ct42_Durchnumerieren(l1);
      if (TEX_OUTPUT) IO_DruckeFlex("%*\\Inst{%%\n", 2*ct42_level);
      ct42_level++;
      if (ct42_equalUptoPos(l,TO_TermAnStelle(l,pos),l1)){
        if(0)IO_DruckeFlex("EQUAL UP TO POS: %t %t (%P == %d == %t)\n",
                      l, l1, l, TO_TermAnStelle(l,ct42_getPos(h)), 
                      ct42_getPos(h), TO_TermAnStelle(l,ct42_getPos(h)));
        h1 = ct42_mkHint(LeafHint|RewrHint|PathHint|ct42_doSplit(h),
                        ct42_getPos(h),ct42_getNo(h));
      }
      else {
        h1 = ct42_mkHint(LeafHint|RewrHint|ct42_doSplit(h),lambdaPos,0);
      }
      *closed = ct42_handleNode(l1,c1,NULL,h1);
      CO_deepDelete(c1);
      TO_TermLoeschen(l1);
      handled = TRUE;
    }
  }
  CLOSETRACE (("ct42_Inst: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct42_XRew (LiteralT l, ContextT c, EConsT e,
                         HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct42_XRew: %t  %C %E\n", l, c, e));
  {
    GleichungsT gleichung;
    unsigned int ppos   = ct42_getPos(h);
    unsigned int endNum = 0;
    unsigned int pos    = ct42_doPath(h) ? 1 : ppos;
    unsigned int no     = ct42_doPath(h) ? 0 : ct42_getNo(h);
    unsigned int no2;
    UTermT l1           = TO_TermAnStelle(l,pos);
    UTermT l0           = TO_NULLTerm(l);
    if (ct42_doCRPath(h) && ct42_doPath(h) && (ppos != lambdaPos)){
    ...
    do {
      if ((endNum != 0) /*&& (pos < ppos)*/ && (TO_GetId(TO_TermEnde(l1)) < endNum)){ 
        if(0)IO_DruckeFlex("Skip position %P (%t) because %d < %d last rewrite at %t (%d) = %P\n",
                      ...
                      TO_TermAnStelle(l,ct42_getPos(h)), TO_GetId(TO_TermAnStelle(l,ct42_getPos(h))),
                      l, TO_TermAnStelle(l,ct42_getPos(h)));
        if (0){
          l1 = TO_Schwanz(l1);
          pos++;
        }
        ...
        if (TO_TermIstNichtVar(l1)){
          if (no == 0 && 
            ...
            HintT h1 = ct42_mkHint(LeafHint|RewrHint|PathHint|ct42_doSplit(h),
                                   pos,0);
            handled = TRUE;
            ct42_Durchnumerieren(lneu);
            if (TEX_OUTPUT) {
            ...
            ct42_level++;
            *closed = ct42_handleNode(lneu,c,e,h1);
            ...
            while (!handled && 
              ...
              if (no2 <= no){
                ...
                if (CO_satisfiableUnder(e1,c)){
                  ...
                  HintT h1 = ct42_mkHint(LeafHint|PathHint|CRPathHint|SplitHint,pos,0);
                  HintT h2 = ct42_mkHint(CrewHint|SplitHint,pos,no2);
                  handled = TRUE;
                  if (TEX_OUTPUT){
                  ...
                  ct42_level++;
                  *closed = PA_preferD1()
                    ? ct42_handleNode(lneu,c,e1,h1) && ct42_handleNode(l,c,e2,h2)
                    : ct42_handleNode(l,c,e2,h2) && ct42_handleNode(lneu,c,e1,h1);
                  CO_deleteEcons(e2);
                }
                else {
                  if (TEX_OUTPUT_F){
                    IO_DruckeFlex("%*\\FCRewrite{%d%s}{%P}{%E \\wedge %C}%%\n", 
                                  2*ct42_level, RE_AbsNr(gleichung), 
                                  TP_RichtungAusgezeichnet(gleichung) ? "" :"'",
                                  l, l1, e1, c);
                  }
                }
  ...
  CLOSETRACE (("ct42_XRew -> %b\n", handled));
  return handled;
}

/* Format von ct42_handleNode
 * static 
...
static BOOLEAN ct42_handleNode (LiteralT l, ContextT c, EConsT e, HintT h)
{
  ...
  OPENTRACE (("ct42_handleNode: %t %C %E\n", l, c, e));
  {
    unsigned int oldlevel = ct42_level;
    ct42_node_counter++;
    if (TEX_OUTPUT) {
      IO_DruckeFlex("%*\\Node{%t}{%t}{%E%C}{%%\n", 
                    2*ct42_level, TO_LitLHS(l), TO_LitRHS(l), e, c);
    }
    ct42_level++;
    if (LIMIT_DEPTH && (ct42_level > DEPTH_LIMIT)){
      /* to catch endless recursion */
      our_error("ct42_handleNode: nesting to deep");
    }
    else if ((PA_maxNodeCount() != 0) && 
             (ct42_node_counter >= PA_maxNodeCount())){
      if (TEX_OUTPUT) IO_DruckeFlex("%*\\ToManyNodes%%\n", 2*ct42_level);
      closed = FALSE;
    }
    else if (ct42_doLeaf(h) && ct42_LeafTests(l,c,e,h,&closed)){
    }
    else if (ct42_doInst(h)  && ct42_Inst(l,c,e,h,&closed)){
    }
    else if (ct42_Split(l,c,e,h,&closed)){
    }
    else if (ct42_XRew(l,c,e,h,&closed)){
    }
    else {
      if (TEX_OUTPUT) IO_DruckeFlex("%*\\GiveUp%%\n", 2*ct42_level);
      closed = FALSE; /* Machen wir's nochmals explizit */
    }
    while (ct42_level > oldlevel){
      ct42_level--;
      if (TEX_OUTPUT) IO_DruckeFlex("%*}%%\n", 2*ct42_level);
    }
  }
  CLOSETRACE (("ct42_handleNode: -> %b\n", closed));
  return closed;
}
...
BOOLEAN CO_GroundJoinable42(TermT s, TermT t)
{
  BOOLEAN res = FALSE;
  {
    ...
    LiteralT l;
    ContextT c;
    HintT h = ct42_mkHint(CrewHint,lambdaPos,0);
    ss = TO_Termkopie(s);
    tt = TO_Termkopie(t);
    TO_mkLit(l,ss,tt);
    ct42_Durchnumerieren(l);
    c = CO_newContext(CO_allocBindings());
    ct42_tree_counter++;
    ct42_node_counter = 0;
    if(TEX_OUTPUT) IO_DruckeFlex("\\ctree{%d}{%%\n", ct42_tree_counter);
    ct42_level++;
    res = ct42_handleNode(l,c,NULL,h);
    if(0)IO_DruckeFlex("#%l %b\n", ct42_node_counter, res);

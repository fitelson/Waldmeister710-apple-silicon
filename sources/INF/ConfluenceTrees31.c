...
static unsigned int  ct31_level = 0;
static unsigned int  ct31_tree_counter = 0;
static unsigned long ct31_node_counter = 0;
...
static HintT ct31_mkHint(unsigned long h, unsigned long pos, unsigned no)
{
...
static BOOLEAN ct31_doLeaf(HintT h)
{
  //  return 1;
  return h & LeafHint;
}

static BOOLEAN ct31_doInst(HintT h)
{
  //  return 1;
  return h & InstHint;
}

static BOOLEAN ct31_doSplit(HintT h)
{
  //  return 1;
  return h & SplitHint;
}

static BOOLEAN ct31_doRewr(HintT h)
{
  //  return 1;
  return h & RewrHint;
}

static BOOLEAN ct31_doPath(HintT h)
{
  //  return 0;
  return h & PathHint;
}

static BOOLEAN ct31_doCRPath(HintT h)
{
  //  return 0;
  return h & CRPathHint;
}

static unsigned long ct31_getPos(HintT h)
{
  //  return 0;
...
static unsigned long ct31_getNo(HintT h)
{
  //  return 0;
...
static BOOLEAN ct31_handleNode (LiteralT l, ContextT c, EConsT e, HintT h);

/* -------------------------------------------------------------------------- */
/* - generische Hilfsroutingen ---------------------------------------------- */
/* -------------------------------------------------------------------------- */

static void ct31_print_DConsKetten(DConsT d,DConsT d1)
{
...
static BOOLEAN ct31_equalUptoPos(LiteralT l1, UTermT p, LiteralT l2)
{
...
static void ct31_Durchnumerieren(UTermT t)
{
...
static BOOLEAN ct31_LeafTests(LiteralT l, 
                             ...
                             BOOLEAN *closed)
{
  ...
  OPENTRACE (("ct31_LeafTests: %t %C %E\n", l, c, e));
  {
    TermT s = TO_LitLHS(l);
    TermT t = TO_LitRHS(l);
    if (TO_TermeGleich(s,t)){
      ...
      if (TEX_OUTPUT) {
        IO_DruckeFlex("%*\\Subsumed{%d}%%\n", 
                      2*ct31_level, RE_AbsNr(MO_AngewendetesObjekt));
      }
      handled = *closed = TRUE;
    }
  ...
  CLOSETRACE (("ct31_LeafTests: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct31_Split(LiteralT l, ContextT c, EConsT e,
                         HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct31_Split: %t %C %E\n", l, c, e));
  {
    if (e != NULL){
      ...
      if (d == NULL){ /* OK, vorhin wurde andere Seite bei CRewrite */
        *closed = TRUE;   /* als erfuellbar nachgewiesen. */
        ...
        HintT h1 = ct31_mkHint(InstHint|RewrHint,lambdaPos,0);
        DConsT drun;
        DConsT d1 = d;
        BOOLEAN allclosed = TRUE;
        if (PA_optDCons()){
          d1 = CO_minimizeDCons(d);
          ct31_print_DConsKetten(d,d1);
        }
        if (PA_sortDCons()){
          CO_sortDCons(d1);
        }
        ...
        do {
          allclosed = ct31_handleNode(l,drun->c,NULL,h1);
          drun = drun->Nachf;
  ...
  CLOSETRACE (("ct31_Split: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct31_Inst(LiteralT l, ContextT c, EConsT e,
                        HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct31_Inst: %t %C %E\n", l, c, e));
  {
    if (CO_somethingBound(c->bindings)){
      HintT h1;
      unsigned int pos = ct31_getPos(h);
      LiteralT l1 = CO_substitute(c->bindings,l);
      ContextT c1 = CO_solveEQPart(c);
      if (e != NULL){
        our_error("rethink for e != NULL!");
      }
      ct31_Durchnumerieren(l1);
      if (TEX_OUTPUT) IO_DruckeFlex("%*\\Inst{%%\n", 2*ct31_level);
      ct31_level++;
      h1 = ct31_mkHint(LeafHint|RewrHint,lambdaPos,0);
      *closed = ct31_handleNode(l1,c1,NULL,h1);
      CO_deepDelete(c1);
      TO_TermLoeschen(l1);
      handled = TRUE;
    }
  }
  CLOSETRACE (("ct31_Inst: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct31_XRew (LiteralT l, ContextT c, EConsT e,
                         HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct31_XRew: %t  %C %E\n", l, c, e));
  {
    GleichungsT gleichung;
    unsigned int ppos   = ct31_getPos(h);
    unsigned int endNum = 0;
    unsigned int pos    = ct31_doPath(h) ? 1 : ppos;
    unsigned int no     = ct31_doPath(h) ? 0 : ct31_getNo(h);
    unsigned int no2;
    UTermT l1           = TO_TermAnStelle(l,pos);
    UTermT l0           = TO_NULLTerm(l);
    if (ct31_doCRPath(h) && ct31_doPath(h) && (ppos != lambdaPos)){
    ...
    do {
      if ((endNum != 0) /*&& (pos < ppos)*/ && (TO_GetId(TO_TermEnde(l1)) < endNum)){ 
        if(0)IO_DruckeFlex("Skip position %P (%t) because %d < %d last rewrite at %t (%d) = %P\n",
                      ...
                      TO_TermAnStelle(l,ct31_getPos(h)), TO_GetId(TO_TermAnStelle(l,ct31_getPos(h))),
                      l, TO_TermAnStelle(l,ct31_getPos(h)));
        if (0){
          l1 = TO_Schwanz(l1);
          pos++;
        }
        ...
        if (TO_TermIstNichtVar(l1)){
          if (no == 0 && 
            ...
            HintT h1 = ct31_mkHint(LeafHint|RewrHint,lambdaPos,0);
            handled = TRUE;
            ct31_Durchnumerieren(lneu);
            if (TEX_OUTPUT) {
            ...
            ct31_level++;
            *closed = ct31_handleNode(lneu,c,e,h1);
            ...
            while (!handled && 
              ...
              if (no2 <= no){
                ...
                if (CO_satisfiableUnder(e1,c)){
                  ...
                  HintT h1 = ct31_mkHint(LeafHint|RewrHint,lambdaPos,0);
                  HintT h2 = ct31_mkHint(CrewHint,pos,no2);
                  handled = TRUE;
                  if (TEX_OUTPUT){
                  ...
                  ct31_level++;
                  *closed = PA_preferD1()
                    ? ct31_handleNode(lneu,c,e1,h1) && ct31_handleNode(l,c,e2,h2)
                    : ct31_handleNode(l,c,e2,h2) && ct31_handleNode(lneu,c,e1,h1);
                  CO_deleteEcons(e2);
                }
                else {
                  if (TEX_OUTPUT_F){
                    IO_DruckeFlex("%*\\FCRewrite{%d%s}{%P}{%E \\wedge %C}%%\n", 
                                  2*ct31_level, RE_AbsNr(gleichung), 
                                  TP_RichtungAusgezeichnet(gleichung) ? "" :"'",
                                  l, l1, e1, c);
                  }
                }
  ...
  CLOSETRACE (("ct31_XRew -> %b\n", handled));
  return handled;
}

/* Format von ct31_handleNode
 * static 
...
static BOOLEAN ct31_handleNode (LiteralT l, ContextT c, EConsT e, HintT h)
{
  ...
  OPENTRACE (("ct31_handleNode: %t %C %E\n", l, c, e));
  {
    unsigned int oldlevel = ct31_level;
    ct31_node_counter++;
    if (TEX_OUTPUT) {
      IO_DruckeFlex("%*\\Node{%t}{%t}{%E%C}{%%\n", 
                    2*ct31_level, TO_LitLHS(l), TO_LitRHS(l), e, c);
    }
    ct31_level++;
    if (LIMIT_DEPTH && (ct31_level > DEPTH_LIMIT)){
      /* to catch endless recursion */
      our_error("ct31_handleNode: nesting to deep");
    }
    else if ((PA_maxNodeCount() != 0) && 
             (ct31_node_counter >= PA_maxNodeCount())){
      if (TEX_OUTPUT) IO_DruckeFlex("%*\\ToManyNodes%%\n", 2*ct31_level);
      closed = FALSE;
    }
    else if (ct31_doLeaf(h) && ct31_LeafTests(l,c,e,h,&closed)){
    }
    else if (ct31_doInst(h)  && ct31_Inst(l,c,e,h,&closed)){
    }
    else if (ct31_XRew(l,c,e,h,&closed)){
    }
    else if (ct31_Split(l,c,e,h,&closed)){
    }
    else {
      if (TEX_OUTPUT) IO_DruckeFlex("%*\\GiveUp%%\n", 2*ct31_level);
      closed = FALSE; /* Machen wir's nochmals explizit */
    }
    while (ct31_level > oldlevel){
      ct31_level--;
      if (TEX_OUTPUT) IO_DruckeFlex("%*}%%\n", 2*ct31_level);
    }
  }
  CLOSETRACE (("ct31_handleNode: -> %b\n", closed));
  return closed;
}
...
BOOLEAN CO_GroundJoinable31(TermT s, TermT t)
{
  BOOLEAN res = FALSE;
  {
    ...
    LiteralT l;
    ContextT c;
    HintT h = ct31_mkHint(CrewHint,lambdaPos,0);
    ss = TO_Termkopie(s);
    tt = TO_Termkopie(t);
    TO_mkLit(l,ss,tt);
    ct31_Durchnumerieren(l);
    c = CO_newContext(CO_allocBindings());
    ct31_tree_counter++;
    ct31_node_counter = 0;
    if(TEX_OUTPUT) IO_DruckeFlex("\\ctree{%d}{%%\n", ct31_tree_counter);
    ct31_level++;
    res = ct31_handleNode(l,c,NULL,h);
    if(0)IO_DruckeFlex("#%l %b\n", ct31_node_counter, res);

...
static unsigned int  ct50_level = 0;
static unsigned int  ct50_tree_counter = 0;
static unsigned long ct50_node_counter = 0;
...
static HintT ct50_mkHint(unsigned long h, unsigned long pos, unsigned no)
{
...
static BOOLEAN ct50_doLeaf(HintT h)
{
  //  return 1;
  return h & LeafHint;
}

static BOOLEAN ct50_doInst(HintT h)
{
  //  return 1;
  return h & InstHint;
}

static BOOLEAN ct50_doSplit(HintT h)
{
  //  return 1;
  return h & SplitHint;
}

static BOOLEAN ct50_doRewr(HintT h)
{
  //  return 1;
  return h & RewrHint;
}

static BOOLEAN ct50_doPath(HintT h)
{
  //  return 0;
  return h & PathHint;
}

static BOOLEAN ct50_doCRPath(HintT h)
{
  //  return 0;
  return h & CRPathHint;
}

static unsigned long ct50_getPos(HintT h)
{
  //  return 0;
...
static unsigned long ct50_getNo(HintT h)
{
  //  return 0;
...
static BOOLEAN ct50_handleNode (LiteralT l, ContextT c, EConsT e, HintT h);

/* -------------------------------------------------------------------------- */
/* - generische Hilfsroutingen ---------------------------------------------- */
/* -------------------------------------------------------------------------- */

static void ct50_print_DConsKetten(DConsT d,DConsT d1)
{
...
static BOOLEAN ct50_equalUptoPos(LiteralT l1, UTermT p, LiteralT l2)
{
...
static void ct50_Durchnumerieren(UTermT t)
{
...
static BOOLEAN ct50_LeafTests(LiteralT l, 
                             ...
                             BOOLEAN *closed)
{
  ...
  OPENTRACE (("ct50_LeafTests: %t %C %E\n", l, c, e));
  {
    TermT s = TO_LitLHS(l);
    TermT t = TO_LitRHS(l);
    if (TO_TermeGleich(s,t)){
      ...
      if (TEX_OUTPUT) {
        IO_DruckeFlex("%*\\Subsumed{%d}%%\n", 
                      2*ct50_level, RE_AbsNr(MO_AngewendetesObjekt));
      }
      handled = *closed = TRUE;
    }
  ...
  CLOSETRACE (("ct50_LeafTests: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct50_Split(LiteralT l, ContextT c, EConsT e,
                         HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct50_Split: %t %C %E\n", l, c, e));
  {
    if (e != NULL){
      ...
      if (d == NULL){ /* OK, vorhin wurde andere Seite bei CRewrite */
        *closed = TRUE;   /* als erfuellbar nachgewiesen. */
        ...
        do {
          allclosed = ct50_handleNode(l,drun->c,NULL,h1);
          drun = drun->Nachf;
  ...
  CLOSETRACE (("ct50_Split: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct50_Inst(LiteralT l, ContextT c, EConsT e,
                        HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct50_Inst: %t %C %E\n", l, c, e));
  {
    if (CO_somethingBound(c->bindings)){
      ...
      ct50_level++;
      *closed = ct50_handleNode(l1,c1,NULL,h1);
      CO_deepDelete(c1);
      TO_TermLoeschen(l1);
      handled = TRUE;
    }
  }
  CLOSETRACE (("ct50_Inst: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct50_Rewrite(LiteralT l, ContextT c, EConsT e,
                           HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct50_Rewrite: %t %C %E\n", l, c, e));
  {
    UTermT l1           = l;
    ...
    do {
        if (TO_TermIstNichtVar(l1) &&
          ...
          ct50_level++;
          *closed = ct50_handleNode(lneu,c,e,h1);
  ...
  CLOSETRACE (("ct50_Rewrite: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct50_CRew (LiteralT l, ContextT c, EConsT e,
                         HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct50_CRew: %t  %C %E\n", l, c, e));
  {
        ...
        if (TO_TermIstNichtVar(l1)){
          ...
          while (!handled && 
              ...
              if (CO_satisfiableUnder(e1,c)){
                ...
                ct50_level++;
                *closed = PA_preferD1()
                  ? ct50_handleNode(lneu,c,e1,h1) && ct50_handleNode(l,c,e2,h2)
                  : ct50_handleNode(l,c,e2,h2) && ct50_handleNode(lneu,c,e1,h1);
                CO_deleteEcons(e2);
              }
              else {
                if (TEX_OUTPUT_F){
                  IO_DruckeFlex("%*\\FCRewrite{%d%s}{%P}{%E \\wedge %C}%%\n", 
                                2*ct50_level, RE_AbsNr(gleichung), 
                                TP_RichtungAusgezeichnet(gleichung) ? "" :"'",
                                l, l1, e1, c);
                }
              }
  ...
  CLOSETRACE (("ct50_CRew -> %b\n", handled));
  return handled;
}

/* Format von ct50_handleNode
 * static 
...
static BOOLEAN ct50_handleNode (LiteralT l, ContextT c, EConsT e, HintT h)
{
  ...
  OPENTRACE (("ct50_handleNode: %t %C %E\n", l, c, e));
  {
    unsigned int oldlevel = ct50_level;
    ct50_node_counter++;
    if (TEX_OUTPUT) {
      IO_DruckeFlex("%*\\Node{%t}{%t}{%E%C}{%%\n", 
                    2*ct50_level, TO_LitLHS(l), TO_LitRHS(l), e, c);
    }
    ct50_level++;
    if (LIMIT_DEPTH && (ct50_level > DEPTH_LIMIT)){
      /* to catch endless recursion */
      our_error("ct50_handleNode: nesting to deep");
    }
    else if ((PA_maxNodeCount() != 0) && 
             (ct50_node_counter >= PA_maxNodeCount())){
      if (TEX_OUTPUT) IO_DruckeFlex("%*\\ToManyNodes%%\n", 2*ct50_level);
      closed = FALSE;
    }
    else if (ct50_LeafTests(l,c,e,h,&closed)){
    }
    else if (ct50_Inst(l,c,e,h,&closed)){
    }
    else if (ct50_Rewrite(l,c,e,h,&closed)){
    }
    else if (ct50_CRew(l,c,e,h,&closed)){
    }
    else if (ct50_Split(l,c,e,h,&closed)){
    }
    else {
      if (TEX_OUTPUT) IO_DruckeFlex("%*\\GiveUp%%\n", 2*ct50_level);
      closed = FALSE; /* Machen wir's nochmals explizit */
    }
    while (ct50_level > oldlevel){
      ct50_level--;
      if (TEX_OUTPUT) IO_DruckeFlex("%*}%%\n", 2*ct50_level);
    }
  }
  CLOSETRACE (("ct50_handleNode: -> %b\n", closed));
  return closed;
}

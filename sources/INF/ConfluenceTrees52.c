...
static unsigned int  ct52_level = 0;
static unsigned int  ct52_tree_counter = 0;
static unsigned long ct52_node_counter = 0;
...
static HintT ct52_mkHint(unsigned long h, unsigned long pos, unsigned no)
{
...
static BOOLEAN ct52_doLeaf(HintT h)
{
  //  return 1;
  return h & LeafHint;
}

static BOOLEAN ct52_doInst(HintT h)
{
  //  return 1;
  return h & InstHint;
}

static BOOLEAN ct52_doSplit(HintT h)
{
  //  return 1;
  return h & SplitHint;
}

static BOOLEAN ct52_doRewr(HintT h)
{
  //  return 1;
  return h & RewrHint;
}

static BOOLEAN ct52_doPath(HintT h)
{
  //  return 0;
  return h & PathHint;
}

static BOOLEAN ct52_doCRPath(HintT h)
{
  //  return 0;
  return h & CRPathHint;
}

static unsigned long ct52_getPos(HintT h)
{
  //  return 0;
...
static unsigned long ct52_getNo(HintT h)
{
  //  return 0;
...
static BOOLEAN ct52_handleNode (LiteralT l, ContextT c, EConsT e, HintT h);

/* -------------------------------------------------------------------------- */
/* - generische Hilfsroutingen ---------------------------------------------- */
/* -------------------------------------------------------------------------- */

static void ct52_print_DConsKetten(DConsT d,DConsT d1)
{
...
static BOOLEAN ct52_equalUptoPos(LiteralT l1, UTermT p, LiteralT l2)
{
...
static void ct52_Durchnumerieren(UTermT t)
{
...
static BOOLEAN ct52_LeafTests(LiteralT l, 
                             ...
                             BOOLEAN *closed)
{
  ...
  OPENTRACE (("ct52_LeafTests: %t %C %E\n", l, c, e));
  {
    TermT s = TO_LitLHS(l);
    TermT t = TO_LitRHS(l);
    if (TO_TermeGleich(s,t)){
      ...
      if (TEX_OUTPUT) {
        IO_DruckeFlex("%*\\Subsumed{%d}%%\n", 
                      2*ct52_level, RE_AbsNr(MO_AngewendetesObjekt));
      }
      handled = *closed = TRUE;
    }
  ...
  CLOSETRACE (("ct52_LeafTests: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct52_Split(LiteralT l, ContextT c, EConsT e,
                         HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct52_Split: %t %C %E\n", l, c, e));
  {
    if (e != NULL){
      ...
      if (d == NULL){ /* OK, vorhin wurde andere Seite bei CRewrite */
        *closed = TRUE;   /* als erfuellbar nachgewiesen. */
        ...
        HintT h1 = ct52_mkHint(InstHint|RewrHint|ct52_doPath(h)|ct52_doCRPath(h),
                              ct52_getPos(h),ct52_getNo(h));
        DConsT drun;
        DConsT d1 = d;
        BOOLEAN allclosed = TRUE;
        if (PA_optDCons()){
          d1 = CO_minimizeDCons(d);
          ct52_print_DConsKetten(d,d1);
        }
        if (PA_sortDCons()){
          CO_sortDCons(d1);
        }
        ...
        do {
          allclosed = ct52_handleNode(l,drun->c,NULL,h1);
          drun = drun->Nachf;
  ...
  CLOSETRACE (("ct52_Split: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct52_Inst(LiteralT l, ContextT c, EConsT e,
                        HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct52_Inst: %t %C %E\n", l, c, e));
  {
    if (CO_somethingBound(c->bindings)){
      HintT h1;
      unsigned int pos = ct52_getPos(h);
      LiteralT l1 = CO_substitute(c->bindings,l);
      ContextT c1 = CO_solveEQPart(c);
      if (e != NULL){
        our_error("rethink for e != NULL!");
      }
      ct52_Durchnumerieren(l1);
      if (TEX_OUTPUT) IO_DruckeFlex("%*\\Inst{%%\n", 2*ct52_level);
      ct52_level++;
      if (ct52_equalUptoPos(l,TO_TermAnStelle(l,pos),l1)){
        if(0)IO_DruckeFlex("EQUAL UP TO POS: %t %t (%P == %d == %t)\n",
                      l, l1, l, TO_TermAnStelle(l,ct52_getPos(h)), 
                      ct52_getPos(h), TO_TermAnStelle(l,ct52_getPos(h)));
        h1 = ct52_mkHint(LeafHint|RewrHint|PathHint|ct52_doSplit(h),
                        ct52_getPos(h),ct52_getNo(h));
      }
      else {
        h1 = ct52_mkHint(LeafHint|RewrHint|ct52_doSplit(h),lambdaPos,0);
      }
      *closed = ct52_handleNode(l1,c1,NULL,h1);
      CO_deepDelete(c1);
      TO_TermLoeschen(l1);
      handled = TRUE;
    }
  }
  CLOSETRACE (("ct52_Inst: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct52_Rewrite(LiteralT l, ContextT c, EConsT e,
                           HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct52_Rewrite: %t %C %E\n", l, c, e));
  {
    unsigned int ppos   = ct52_getPos(h);
    unsigned int endNum = 0;
    unsigned int pos    = ct52_doPath(h) ? 1 : ppos;
    ...
    if (ct52_doPath(h) && (ppos != lambdaPos)){
    ...
    do {
      if ((endNum != 0) /*&& (pos < ppos)*/ && (TO_GetId(TO_TermEnde(l1)) < endNum)){ 
        if(0)IO_DruckeFlex("Skip position %P (%t) because %d < %d last rewrite at %t (%d) = %P\n",
                      ...
                      TO_TermAnStelle(l,ct52_getPos(h)), TO_GetId(TO_TermAnStelle(l,ct52_getPos(h))),
                      l, TO_TermAnStelle(l,ct52_getPos(h)));
        if (0){
          l1 = TO_Schwanz(l1);
          pos++;
        }
        ...
        if (TO_TermIstNichtVar(l1) &&
          ...
          HintT h1 = ct52_mkHint(LeafHint|RewrHint|PathHint|ct52_doSplit(h),
                                pos,0);
          handled = TRUE;
          ct52_Durchnumerieren(lneu);
          if (TEX_OUTPUT) {
          ...
          ct52_level++;
          *closed = ct52_handleNode(lneu,c,e,h1);
  ...
  CLOSETRACE (("ct52_Rewrite: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct52_CRew (LiteralT l, ContextT c, EConsT e,
                         HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct52_CRew: %t  %C %E\n", l, c, e));
  {
    GleichungsT gleichung;
    unsigned int ppos   = ct52_getPos(h);
    unsigned int endNum = 0;
    unsigned int pos    = ct52_doPath(h) ? 1 : ppos;
    unsigned int no     = ct52_doPath(h) ? 0 : ct52_getNo(h);
    unsigned int no2;
    UTermT l1           = TO_TermAnStelle(l,pos);
    UTermT l0           = TO_NULLTerm(l);
    if (ct52_doCRPath(h) && ct52_doPath(h) && (ppos != lambdaPos)){
      endNum = TO_GetId(TO_TermEnde(TO_TermAnStelle(l,ppos)));
    }
    do {
      if ((endNum != 0) /*&& (pos < ppos)*/ && (TO_GetId(TO_TermEnde(l1)) < endNum)){ 
          ...
          while (!handled && 
            ...
            if (no2 <= no){
              ...
              if (CO_satisfiableUnder(e1,c)){
                ...
                HintT h1 = ct52_mkHint(LeafHint|RewrHint|PathHint|CRPathHint|SplitHint,pos,0);
                HintT h2 = ct52_mkHint(CrewHint|SplitHint,pos,no2);
                handled = TRUE;
                if (TEX_OUTPUT){
                ...
                ct52_level++;
                *closed = PA_preferD1()
                  ? ct52_handleNode(lneu,c,e1,h1) && ct52_handleNode(l,c,e2,h2)
                  : ct52_handleNode(l,c,e2,h2) && ct52_handleNode(lneu,c,e1,h1);
                CO_deleteEcons(e2);
              }
              else {
                if (TEX_OUTPUT_F){
                  IO_DruckeFlex("%*\\FCRewrite{%d%s}{%P}{%E \\wedge %C}%%\n", 
                                2*ct52_level, RE_AbsNr(gleichung), 
                                TP_RichtungAusgezeichnet(gleichung) ? "" :"'",
                                l, l1, e1, c);
                }
              }
  ...
  CLOSETRACE (("ct52_CRew -> %b\n", handled));
  return handled;
}

/* Format von ct52_handleNode
 * static 
...
static BOOLEAN ct52_handleNode (LiteralT l, ContextT c, EConsT e, HintT h)
{
  ...
  OPENTRACE (("ct52_handleNode: %t %C %E\n", l, c, e));
  {
    unsigned int oldlevel = ct52_level;
    ct52_node_counter++;
    if (TEX_OUTPUT) {
      IO_DruckeFlex("%*\\Node{%t}{%t}{%E%C}{%%\n", 
                    2*ct52_level, TO_LitLHS(l), TO_LitRHS(l), e, c);
    }
    ct52_level++;
    if (LIMIT_DEPTH && (ct52_level > DEPTH_LIMIT)){
      /* to catch endless recursion */
      our_error("ct52_handleNode: nesting to deep");
    }
    else if ((PA_maxNodeCount() != 0) && 
             (ct52_node_counter >= PA_maxNodeCount())){
      if (TEX_OUTPUT) IO_DruckeFlex("%*\\ToManyNodes%%\n", 2*ct52_level);
      closed = FALSE;
    }
    else if (ct52_doLeaf(h) && ct52_LeafTests(l,c,e,h,&closed)){
    }
    else if (ct52_doInst(h)  && ct52_Inst(l,c,e,h,&closed)){
    }
    else if (ct52_doRewr(h)  && ct52_Rewrite(l,c,e,h,&closed)){
    }
    else if (ct52_CRew(l,c,e,h,&closed)){
    }
    else if (ct52_Split(l,c,e,h,&closed)){
    }
    else {
      if (TEX_OUTPUT) IO_DruckeFlex("%*\\GiveUp%%\n", 2*ct52_level);
      closed = FALSE; /* Machen wir's nochmals explizit */
    }
    while (ct52_level > oldlevel){
      ct52_level--;
      if (TEX_OUTPUT) IO_DruckeFlex("%*}%%\n", 2*ct52_level);
    }
  }
  CLOSETRACE (("ct52_handleNode: -> %b\n", closed));
  return closed;
}
...
BOOLEAN CO_GroundJoinable52(TermT s, TermT t)
{
  BOOLEAN res = FALSE;
  {
    ...
    LiteralT l;
    ContextT c;
    HintT h = ct52_mkHint(CrewHint,lambdaPos,0);
    ss = TO_Termkopie(s);
    tt = TO_Termkopie(t);
    TO_mkLit(l,ss,tt);
    ct52_Durchnumerieren(l);
    c = CO_newContext(CO_allocBindings());
    ct52_tree_counter++;
    ct52_node_counter = 0;
    if(TEX_OUTPUT) IO_DruckeFlex("\\ctree{%d}{%%\n", ct52_tree_counter);
    ct52_level++;
    res = ct52_handleNode(l,c,NULL,h);
    if(0)IO_DruckeFlex("#%l %b\n", ct52_node_counter, res);

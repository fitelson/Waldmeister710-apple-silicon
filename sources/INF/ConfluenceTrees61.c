...
static unsigned int  ct61_level = 0;
static unsigned int  ct61_tree_counter = 0;
static unsigned long ct61_node_counter = 0;
...
static HintT ct61_mkHint(unsigned long h, unsigned long pos, unsigned no)
{
...
static BOOLEAN ct61_doLeaf(HintT h)
{
  //  return 1;
  return h & LeafHint;
}

static BOOLEAN ct61_doInst(HintT h)
{
  //  return 1;
  return h & InstHint;
}

static BOOLEAN ct61_doSplit(HintT h)
{
  //  return 1;
  return h & SplitHint;
}

static BOOLEAN ct61_doRewr(HintT h)
{
  //  return 1;
  return h & RewrHint;
}

static BOOLEAN ct61_doPath(HintT h)
{
  //  return 0;
  return h & PathHint;
}

static BOOLEAN ct61_doCRPath(HintT h)
{
  //  return 0;
  return h & CRPathHint;
}

static unsigned long ct61_getPos(HintT h)
{
  //  return 0;
...
static unsigned long ct61_getNo(HintT h)
{
  //  return 0;
...
static BOOLEAN ct61_handleNode (LiteralT l, ContextT c, EConsT e, HintT h);

/* -------------------------------------------------------------------------- */
/* - generische Hilfsroutingen ---------------------------------------------- */
/* -------------------------------------------------------------------------- */

static void ct61_print_DConsKetten(DConsT d,DConsT d1)
{
...
static BOOLEAN ct61_equalUptoPos(LiteralT l1, UTermT p, LiteralT l2)
{
...
static void ct61_Durchnumerieren(UTermT t)
{
...
static BOOLEAN ct61_LeafTests(LiteralT l, 
                             ...
                             BOOLEAN *closed)
{
  ...
  OPENTRACE (("ct61_LeafTests: %t %C %E\n", l, c, e));
  {
    TermT s = TO_LitLHS(l);
    TermT t = TO_LitRHS(l);
    if (TO_TermeGleich(s,t)){
      ...
      if (TEX_OUTPUT) {
        IO_DruckeFlex("%*\\Subsumed{%d}%%\n", 
                      2*ct61_level, RE_AbsNr(MO_AngewendetesObjekt));
      }
      handled = *closed = TRUE;
    }
  ...
  CLOSETRACE (("ct61_LeafTests: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct61_Split(LiteralT l, ContextT c, EConsT e,
                         HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct61_Split: %t %C %E\n", l, c, e));
  {
    if (e != NULL){
      ...
      if (d == NULL){ /* OK, vorhin wurde andere Seite bei CRewrite */
        *closed = TRUE;   /* als erfuellbar nachgewiesen. */
        ...
        HintT h1 = ct61_mkHint(InstHint|RewrHint,lambdaPos,0);
        DConsT drun;
        DConsT d1 = d;
        BOOLEAN allclosed = TRUE;
        if (PA_optDCons()){
          d1 = CO_minimizeDCons(d);
          ct61_print_DConsKetten(d,d1);
        }
        if (PA_sortDCons()){
          CO_sortDCons(d1);
        }
        ...
        do {
          allclosed = ct61_handleNode(l,drun->c,NULL,h1);
          drun = drun->Nachf;
  ...
  CLOSETRACE (("ct61_Split: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct61_Inst(LiteralT l, ContextT c, EConsT e,
                        HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct61_Inst: %t %C %E\n", l, c, e));
  {
    if (CO_somethingBound(c->bindings)){
      HintT h1;
      unsigned int pos = ct61_getPos(h);
      LiteralT l1 = CO_substitute(c->bindings,l);
      ContextT c1 = CO_solveEQPart(c);
      if (e != NULL){
        our_error("rethink for e != NULL!");
      }
      ct61_Durchnumerieren(l1);
      if (TEX_OUTPUT) IO_DruckeFlex("%*\\Inst{%%\n", 2*ct61_level);
      ct61_level++;
      h1 = ct61_mkHint(InstHint|RewrHint,lambdaPos,0);
      *closed = ct61_handleNode(l1,c1,NULL,h1);
      CO_deepDelete(c1);
      TO_TermLoeschen(l1);
      handled = TRUE;
    }
  }
  CLOSETRACE (("ct61_Inst: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct61_Rewrite(LiteralT l, ContextT c, EConsT e,
                           HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct61_Rewrite: %t %C %E\n", l, c, e));
  {
    unsigned int ppos   = ct61_getPos(h);
    unsigned int endNum = 0;
    unsigned int pos    = ct61_doPath(h) ? 1 : ppos;
    ...
    if (ct61_doPath(h) && (ppos != lambdaPos)){
    ...
    do {
      if ((endNum != 0) /*&& (pos < ppos)*/ && (TO_GetId(TO_TermEnde(l1)) < endNum)){ 
        if(0)IO_DruckeFlex("Skip position %P (%t) because %d < %d last rewrite at %t (%d) = %P\n",
                      ...
                      TO_TermAnStelle(l,ct61_getPos(h)), TO_GetId(TO_TermAnStelle(l,ct61_getPos(h))),
                      l, TO_TermAnStelle(l,ct61_getPos(h)));
        if (0){
          l1 = TO_Schwanz(l1);
          pos++;
        }
        ...
        if (TO_TermIstNichtVar(l1) &&
          ...
          HintT h1 = ct61_mkHint(LeafHint|RewrHint|PathHint,pos,0);
          handled = TRUE;
          ct61_Durchnumerieren(lneu);
          if (TEX_OUTPUT) {
          ...
          ct61_level++;
          *closed = ct61_handleNode(lneu,c,e,h1);
  ...
  CLOSETRACE (("ct61_Rewrite: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct61_CRew (LiteralT l, ContextT c, EConsT e,
                         HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct61_CRew: %t  %C %E\n", l, c, e));
  {
    GleichungsT gleichung;
    unsigned int ppos   = ct61_getPos(h);
    unsigned int endNum = 0;
    unsigned int pos    = ct61_doPath(h) ? 1 : ppos;
    unsigned int no     = ct61_doPath(h) ? 0 : ct61_getNo(h);
    unsigned int no2;
    UTermT l1           = TO_TermAnStelle(l,pos);
    UTermT l0           = TO_NULLTerm(l);
    if (ct61_doCRPath(h) && ct61_doPath(h) && (ppos != lambdaPos)){
      endNum = TO_GetId(TO_TermEnde(TO_TermAnStelle(l,ppos)));
    }
    do {
      if ((endNum != 0) /*&& (pos < ppos)*/ && (TO_GetId(TO_TermEnde(l1)) < endNum)){ 
          ...
          while (!handled && 
            ...
            if (no2 <= no){
              ...
              if (CO_satisfiableUnder(e1,c)){
                ...
                HintT h1 = ct61_mkHint(LeafHint|RewrHint,lambdaPos,0);
                HintT h2 = ct61_mkHint(CrewHint,pos,no2);
                handled = TRUE;
                if (TEX_OUTPUT){
                ...
                ct61_level++;
                *closed = PA_preferD1()
                  ? ct61_handleNode(lneu,c,e1,h1) && ct61_handleNode(l,c,e2,h2)
                  : ct61_handleNode(l,c,e2,h2) && ct61_handleNode(lneu,c,e1,h1);
                CO_deleteEcons(e2);
              }
              else {
                if (TEX_OUTPUT_F){
                  IO_DruckeFlex("%*\\FCRewrite{%d%s}{%P}{%E \\wedge %C}%%\n", 
                                2*ct61_level, RE_AbsNr(gleichung), 
                                TP_RichtungAusgezeichnet(gleichung) ? "" :"'",
                                l, l1, e1, c);
                }
              }
  ...
  CLOSETRACE (("ct61_CRew -> %b\n", handled));
  return handled;
}

/* Format von ct61_handleNode
 * static 
...
static BOOLEAN ct61_handleNode (LiteralT l, ContextT c, EConsT e, HintT h)
{
  ...
  OPENTRACE (("ct61_handleNode: %t %C %E\n", l, c, e));
  {
    unsigned int oldlevel = ct61_level;
    ct61_node_counter++;
    if (TEX_OUTPUT) {
      IO_DruckeFlex("%*\\Node{%t}{%t}{%E%C}{%%\n", 
                    2*ct61_level, TO_LitLHS(l), TO_LitRHS(l), e, c);
    }
    ct61_level++;
    if (LIMIT_DEPTH && (ct61_level > DEPTH_LIMIT)){
      /* to catch endless recursion */
      our_error("ct61_handleNode: nesting to deep");
    }
    else if ((PA_maxNodeCount() != 0) && 
             (ct61_node_counter >= PA_maxNodeCount())){
      if (TEX_OUTPUT) IO_DruckeFlex("%*\\ToManyNodes%%\n", 2*ct61_level);
      closed = FALSE;
    }
    else if (ct61_doLeaf(h) && ct61_LeafTests(l,c,e,h,&closed)){
    }
    else if (ct61_doInst(h)  && ct61_Inst(l,c,e,h,&closed)){
    }
    else if (ct61_Split(l,c,e,h,&closed)){
    }
    else if (ct61_doRewr(h)  && ct61_Rewrite(l,c,e,h,&closed)){
    }
    else if (ct61_CRew(l,c,e,h,&closed)){
    }
    else {
      if (TEX_OUTPUT) IO_DruckeFlex("%*\\GiveUp%%\n", 2*ct61_level);
      closed = FALSE; /* Machen wir's nochmals explizit */
    }
    while (ct61_level > oldlevel){
      ct61_level--;
      if (TEX_OUTPUT) IO_DruckeFlex("%*}%%\n", 2*ct61_level);
    }
  }
  CLOSETRACE (("ct61_handleNode: -> %b\n", closed));
  return closed;
}
...
BOOLEAN CO_GroundJoinable61(TermT s, TermT t)
{
  BOOLEAN res = FALSE;
  {
    ...
    LiteralT l;
    ContextT c;
    HintT h = ct61_mkHint(CrewHint,lambdaPos,0);
    ss = TO_Termkopie(s);
    tt = TO_Termkopie(t);
    TO_mkLit(l,ss,tt);
    ct61_Durchnumerieren(l);
    c = CO_newContext(CO_allocBindings());
    ct61_tree_counter++;
    ct61_node_counter = 0;
    if(TEX_OUTPUT) IO_DruckeFlex("\\ctree{%d}{%%\n", ct61_tree_counter);
    ct61_level++;
    res = ct61_handleNode(l,c,NULL,h);
    if(0)IO_DruckeFlex("#%l %b\n", ct61_node_counter, res);

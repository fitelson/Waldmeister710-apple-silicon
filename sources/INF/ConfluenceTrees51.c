...
static unsigned int  ct51_level = 0;
static unsigned int  ct51_tree_counter = 0;
static unsigned long ct51_node_counter = 0;
...
static HintT ct51_mkHint(unsigned long h, unsigned long pos, unsigned no)
{
...
static BOOLEAN ct51_doLeaf(HintT h)
{
  //  return 1;
  return h & LeafHint;
}

static BOOLEAN ct51_doInst(HintT h)
{
  //  return 1;
  return h & InstHint;
}

static BOOLEAN ct51_doSplit(HintT h)
{
  //  return 1;
  return h & SplitHint;
}

static BOOLEAN ct51_doRewr(HintT h)
{
  //  return 1;
  return h & RewrHint;
}

static BOOLEAN ct51_doPath(HintT h)
{
  //  return 0;
  return h & PathHint;
}

static BOOLEAN ct51_doCRPath(HintT h)
{
  //  return 0;
  return h & CRPathHint;
}

static unsigned long ct51_getPos(HintT h)
{
  //  return 0;
...
static unsigned long ct51_getNo(HintT h)
{
  //  return 0;
...
static BOOLEAN ct51_handleNode (LiteralT l, ContextT c, EConsT e, HintT h);

/* -------------------------------------------------------------------------- */
/* - generische Hilfsroutingen ---------------------------------------------- */
/* -------------------------------------------------------------------------- */

static void ct51_print_DConsKetten(DConsT d,DConsT d1)
{
...
static BOOLEAN ct51_equalUptoPos(LiteralT l1, UTermT p, LiteralT l2)
{
...
static void ct51_Durchnumerieren(UTermT t)
{
...
static BOOLEAN ct51_LeafTests(LiteralT l, 
                             ...
                             BOOLEAN *closed)
{
  ...
  OPENTRACE (("ct51_LeafTests: %t %C %E\n", l, c, e));
  {
    TermT s = TO_LitLHS(l);
    TermT t = TO_LitRHS(l);
    if (TO_TermeGleich(s,t)){
      ...
      if (TEX_OUTPUT) {
        IO_DruckeFlex("%*\\Subsumed{%d}%%\n", 
                      2*ct51_level, RE_AbsNr(MO_AngewendetesObjekt));
      }
      handled = *closed = TRUE;
    }
  ...
  CLOSETRACE (("ct51_LeafTests: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct51_Split(LiteralT l, ContextT c, EConsT e,
                         HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct51_Split: %t %C %E\n", l, c, e));
  {
    if (e != NULL){
      ...
      if (d == NULL){ /* OK, vorhin wurde andere Seite bei CRewrite */
        *closed = TRUE;   /* als erfuellbar nachgewiesen. */
        ...
        HintT h1 = ct51_mkHint(InstHint|RewrHint,lambdaPos,0);
        DConsT drun;
        DConsT d1 = d;
        BOOLEAN allclosed = TRUE;
        if (PA_optDCons()){
          d1 = CO_minimizeDCons(d);
          ct51_print_DConsKetten(d,d1);
        }
        if (PA_sortDCons()){
          CO_sortDCons(d1);
        }
        ...
        do {
          allclosed = ct51_handleNode(l,drun->c,NULL,h1);
          drun = drun->Nachf;
  ...
  CLOSETRACE (("ct51_Split: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct51_Inst(LiteralT l, ContextT c, EConsT e,
                        HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct51_Inst: %t %C %E\n", l, c, e));
  {
    if (CO_somethingBound(c->bindings)){
      HintT h1;
      unsigned int pos = ct51_getPos(h);
      LiteralT l1 = CO_substitute(c->bindings,l);
      ContextT c1 = CO_solveEQPart(c);
      if (e != NULL){
        our_error("rethink for e != NULL!");
      }
      ct51_Durchnumerieren(l1);
      if (TEX_OUTPUT) IO_DruckeFlex("%*\\Inst{%%\n", 2*ct51_level);
      ct51_level++;
      h1 = ct51_mkHint(LeafHint|RewrHint,lambdaPos,0);
      *closed = ct51_handleNode(l1,c1,NULL,h1);
      CO_deepDelete(c1);
      TO_TermLoeschen(l1);
      handled = TRUE;
    }
  }
  CLOSETRACE (("ct51_Inst: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct51_Rewrite(LiteralT l, ContextT c, EConsT e,
                           HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct51_Rewrite: %t %C %E\n", l, c, e));
  {
    unsigned int ppos   = ct51_getPos(h);
    unsigned int endNum = 0;
    unsigned int pos    = ct51_doPath(h) ? 1 : ppos;
    ...
    if (ct51_doPath(h) && (ppos != lambdaPos)){
    ...
    do {
      if ((endNum != 0) /*&& (pos < ppos)*/ && (TO_GetId(TO_TermEnde(l1)) < endNum)){ 
        if(0)IO_DruckeFlex("Skip position %P (%t) because %d < %d last rewrite at %t (%d) = %P\n",
                      ...
                      TO_TermAnStelle(l,ct51_getPos(h)), TO_GetId(TO_TermAnStelle(l,ct51_getPos(h))),
                      l, TO_TermAnStelle(l,ct51_getPos(h)));
        if (0){
          l1 = TO_Schwanz(l1);
          pos++;
        }
        ...
        if (TO_TermIstNichtVar(l1) &&
          ...
          HintT h1 = ct51_mkHint(LeafHint|RewrHint,lambdaPos,0);
          handled = TRUE;
          ct51_Durchnumerieren(lneu);
          if (TEX_OUTPUT) {
          ...
          ct51_level++;
          *closed = ct51_handleNode(lneu,c,e,h1);
  ...
  CLOSETRACE (("ct51_Rewrite: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct51_CRew (LiteralT l, ContextT c, EConsT e,
                         HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct51_CRew: %t  %C %E\n", l, c, e));
  {
    GleichungsT gleichung;
    unsigned int ppos   = ct51_getPos(h);
    unsigned int endNum = 0;
    unsigned int pos    = ct51_doPath(h) ? 1 : ppos;
    unsigned int no     = ct51_doPath(h) ? 0 : ct51_getNo(h);
    unsigned int no2;
    UTermT l1           = TO_TermAnStelle(l,pos);
    UTermT l0           = TO_NULLTerm(l);
    if (ct51_doCRPath(h) && ct51_doPath(h) && (ppos != lambdaPos)){
      endNum = TO_GetId(TO_TermEnde(TO_TermAnStelle(l,ppos)));
    }
    do {
      if ((endNum != 0) /*&& (pos < ppos)*/ && (TO_GetId(TO_TermEnde(l1)) < endNum)){ 
          ...
          while (!handled && 
            ...
            if (no2 <= no){
              ...
              if (CO_satisfiableUnder(e1,c)){
                ...
                HintT h1 = ct51_mkHint(LeafHint|RewrHint,lambdaPos,0);
                HintT h2 = ct51_mkHint(CrewHint,pos,no2);
                handled = TRUE;
                if (TEX_OUTPUT){
                ...
                ct51_level++;
                *closed = PA_preferD1()
                  ? ct51_handleNode(lneu,c,e1,h1) && ct51_handleNode(l,c,e2,h2)
                  : ct51_handleNode(l,c,e2,h2) && ct51_handleNode(lneu,c,e1,h1);
                CO_deleteEcons(e2);
              }
              else {
                if (TEX_OUTPUT_F){
                  IO_DruckeFlex("%*\\FCRewrite{%d%s}{%P}{%E \\wedge %C}%%\n", 
                                2*ct51_level, RE_AbsNr(gleichung), 
                                TP_RichtungAusgezeichnet(gleichung) ? "" :"'",
                                l, l1, e1, c);
                }
              }
  ...
  CLOSETRACE (("ct51_CRew -> %b\n", handled));
  return handled;
}

/* Format von ct51_handleNode
 * static 
...
static BOOLEAN ct51_handleNode (LiteralT l, ContextT c, EConsT e, HintT h)
{
  ...
  OPENTRACE (("ct51_handleNode: %t %C %E\n", l, c, e));
  {
    unsigned int oldlevel = ct51_level;
    ct51_node_counter++;
    if (TEX_OUTPUT) {
      IO_DruckeFlex("%*\\Node{%t}{%t}{%E%C}{%%\n", 
                    2*ct51_level, TO_LitLHS(l), TO_LitRHS(l), e, c);
    }
    ct51_level++;
    if (LIMIT_DEPTH && (ct51_level > DEPTH_LIMIT)){
      /* to catch endless recursion */
      our_error("ct51_handleNode: nesting to deep");
    }
    else if ((PA_maxNodeCount() != 0) && 
             (ct51_node_counter >= PA_maxNodeCount())){
      if (TEX_OUTPUT) IO_DruckeFlex("%*\\ToManyNodes%%\n", 2*ct51_level);
      closed = FALSE;
    }
    else if (ct51_doLeaf(h) && ct51_LeafTests(l,c,e,h,&closed)){
    }
    else if (ct51_doInst(h)  && ct51_Inst(l,c,e,h,&closed)){
    }
    else if (ct51_doRewr(h)  && ct51_Rewrite(l,c,e,h,&closed)){
    }
    else if (ct51_CRew(l,c,e,h,&closed)){
    }
    else if (ct51_Split(l,c,e,h,&closed)){
    }
    else {
      if (TEX_OUTPUT) IO_DruckeFlex("%*\\GiveUp%%\n", 2*ct51_level);
      closed = FALSE; /* Machen wir's nochmals explizit */
    }
    while (ct51_level > oldlevel){
      ct51_level--;
      if (TEX_OUTPUT) IO_DruckeFlex("%*}%%\n", 2*ct51_level);
    }
  }
  CLOSETRACE (("ct51_handleNode: -> %b\n", closed));
  return closed;
}
...
BOOLEAN CO_GroundJoinable51(TermT s, TermT t)
{
  BOOLEAN res = FALSE;
  {
    ...
    LiteralT l;
    ContextT c;
    HintT h = ct51_mkHint(CrewHint,lambdaPos,0);
    ss = TO_Termkopie(s);
    tt = TO_Termkopie(t);
    TO_mkLit(l,ss,tt);
    ct51_Durchnumerieren(l);
    c = CO_newContext(CO_allocBindings());
    ct51_tree_counter++;
    ct51_node_counter = 0;
    if(TEX_OUTPUT) IO_DruckeFlex("\\ctree{%d}{%%\n", ct51_tree_counter);
    ct51_level++;
    res = ct51_handleNode(l,c,NULL,h);
    if(0)IO_DruckeFlex("#%l %b\n", ct51_node_counter, res);

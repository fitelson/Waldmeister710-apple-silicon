...
static unsigned int  ct8_level = 0;
static unsigned int  ct8_tree_counter = 0;
static unsigned long ct8_node_counter = 0;
...
static HintT ct8_mkHint(unsigned long h, unsigned long pos, unsigned no)
{
...
static BOOLEAN ct8_doLeaf(HintT h)
{
  //  return 1;
  return h & LeafHint;
}

static BOOLEAN ct8_doInst(HintT h)
{
  //  return 1;
  return h & InstHint;
}

static BOOLEAN ct8_doSplit(HintT h)
{
  //  return 1;
  return h & SplitHint;
}

static BOOLEAN ct8_doRewr(HintT h)
{
  //  return 1;
  return h & RewrHint;
}

static BOOLEAN ct8_doPath(HintT h)
{
  //  return 0;
  return h & PathHint;
}

static BOOLEAN ct8_doCRPath(HintT h)
{
  //  return 0;
  return h & CRPathHint;
}

static unsigned long ct8_getPos(HintT h)
{
  //  return 0;
...
static unsigned long ct8_getNo(HintT h)
{
  //  return 0;
...
static BOOLEAN ct8_handleNode (LiteralT l, ContextT c, EConsT e, HintT h);

/* -------------------------------------------------------------------------- */
/* - generische Hilfsroutingen ---------------------------------------------- */
/* -------------------------------------------------------------------------- */

static void ct8_print_DConsKetten(DConsT d,DConsT d1)
{
...
static BOOLEAN ct8_equalUptoPos(LiteralT l1, UTermT p, LiteralT l2)
{
...
static void ct8_Durchnumerieren(UTermT t)
{
...
static BOOLEAN ct8_LeafTests(LiteralT l, 
                             ...
                             BOOLEAN *closed)
{
    ...
    if (TO_TermeGleich(s,t)){
      ...
      if (TEX_OUTPUT) {
        IO_DruckeFlex("%*\\Subsumed{%d}%%\n", 
                      2*ct8_level, RE_AbsNr(MO_AngewendetesObjekt));
      }
      handled = *closed = TRUE;
    }
  ...
  CLOSETRACE (("ct8_LeafTests: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct8_Split(LiteralT l, ContextT c, EConsT e,
                         HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct8_Split: %t %C %E\n", l, c, e));
  {
    if (e != NULL){
      ...
      if (d == NULL){ /* OK, vorhin wurde andere Seite bei CRewrite */
        *closed = TRUE;   /* als erfuellbar nachgewiesen. */
        ...
        HintT h1 = ct8_mkHint(InstHint|RewrHint|ct8_doPath(h)|ct8_doCRPath(h),
                              ct8_getPos(h),ct8_getNo(h));
        DConsT drun;
        DConsT d1 = d;
        BOOLEAN allclosed = TRUE;
        if (PA_optDCons()){
          d1 = CO_minimizeDCons(d);
          ct8_print_DConsKetten(d,d1);
        }
        if (PA_sortDCons()){
          CO_sortDCons(d1);
        }
        ...
        do {
          allclosed = ct8_handleNode(l,drun->c,NULL,h1);
          drun = drun->Nachf;
  ...
  CLOSETRACE (("ct8_Split: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct8_Inst(LiteralT l, ContextT c, EConsT e,
                        HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct8_Inst: %t %C %E\n", l, c, e));
  {
    if (CO_somethingBound(c->bindings)){
      HintT h1;
      unsigned int pos = ct8_getPos(h);
      LiteralT l1 = CO_substitute(c->bindings,l);
      ContextT c1 = CO_solveEQPart(c);
      if (e != NULL){
        our_error("rethink for e != NULL!");
      }
      ct8_Durchnumerieren(l1);
      if (TEX_OUTPUT) IO_DruckeFlex("%*\\Inst{%%\n", 2*ct8_level);
      ct8_level++;
      if (ct8_equalUptoPos(l,TO_TermAnStelle(l,pos),l1)){
        if(0)IO_DruckeFlex("EQUAL UP TO POS: %t %t (%P == %d == %t)\n",
                      l, l1, l, TO_TermAnStelle(l,ct8_getPos(h)), 
                      ct8_getPos(h), TO_TermAnStelle(l,ct8_getPos(h)));
        h1 = ct8_mkHint(LeafHint|RewrHint|PathHint|ct8_doSplit(h),
                        ct8_getPos(h),ct8_getNo(h));
      }
      else {
        h1 = ct8_mkHint(LeafHint|RewrHint|ct8_doSplit(h),lambdaPos,0);
      }
      *closed = ct8_handleNode(l1,c1,NULL,h1);
      CO_deepDelete(c1);
      TO_TermLoeschen(l1);
      handled = TRUE;
    }
  }
  CLOSETRACE (("ct8_Inst: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct8_Rewrite(LiteralT l, ContextT c, EConsT e,
                           HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct8_Rewrite: %t %C %E\n", l, c, e));
  {
    unsigned int ppos   = ct8_getPos(h);
    unsigned int endNum = 0;
    unsigned int pos    = ct8_doPath(h) ? 1 : ppos;
    ...
    if (ct8_doPath(h) && (ppos != lambdaPos)){
    ...
    do {
      if ((endNum != 0) /*&& (pos < ppos)*/ && (TO_GetId(TO_TermEnde(l1)) < endNum)){ 
        if(0)IO_DruckeFlex("Skip position %P (%t) because %d < %d last rewrite at %t (%d) = %P\n",
                      ...
                      TO_TermAnStelle(l,ct8_getPos(h)), TO_GetId(TO_TermAnStelle(l,ct8_getPos(h))),
                      l, TO_TermAnStelle(l,ct8_getPos(h)));
        if (0){
          l1 = TO_Schwanz(l1);
          pos++;
        }
        ...
        if (TO_TermIstNichtVar(l1) &&
          ...
          HintT h1 = ct8_mkHint(LeafHint|RewrHint|PathHint|ct8_doSplit(h),
                                pos,0);
          handled = TRUE;
          ct8_Durchnumerieren(lneu);
          if (TEX_OUTPUT) {
          ...
          ct8_level++;
          *closed = ct8_handleNode(lneu,c,e,h1);
  ...
  CLOSETRACE (("ct8_Rewrite: -> %b\n", handled));
  return handled;
}

static BOOLEAN ct8_CRew (LiteralT l, ContextT c, EConsT e,
                         HintT h, BOOLEAN *closed)
{
  BOOLEAN handled = FALSE;
  OPENTRACE (("ct8_CRew: %t  %C %E\n", l, c, e));
  {
    GleichungsT gleichung;
    unsigned int ppos   = ct8_getPos(h);
    unsigned int endNum = 0;
    unsigned int pos    = ct8_doPath(h) ? 1 : ppos;
    unsigned int no     = ct8_doPath(h) ? 0 : ct8_getNo(h);
    unsigned int no2;
    UTermT l1           = TO_TermAnStelle(l,pos);
    UTermT l0           = TO_NULLTerm(l);
    if (ct8_doCRPath(h) && ct8_doPath(h) && (ppos != lambdaPos)){
      endNum = TO_GetId(TO_TermEnde(TO_TermAnStelle(l,ppos)));
    }
    do {
      if ((endNum != 0) /*&& (pos < ppos)*/ && (TO_GetId(TO_TermEnde(l1)) < endNum)){ 
          ...
          while (!handled && 
            ...
            if (no2 <= no){
              ...
              if (CO_satisfiableUnder(e1,c)){
                ...
                HintT h1 = ct8_mkHint(LeafHint|PathHint|CRPathHint|SplitHint,pos,0);
                HintT h2 = ct8_mkHint(CrewHint|SplitHint,pos,no2);
                handled = TRUE;
                if (TEX_OUTPUT){
                ...
                ct8_level++;
                *closed = PA_preferD1()
                  ? ct8_handleNode(lneu,c,e1,h1) && ct8_handleNode(l,c,e2,h2)
                  : ct8_handleNode(l,c,e2,h2) && ct8_handleNode(lneu,c,e1,h1);
                CO_deleteEcons(e2);
              }
              else {
                if (TEX_OUTPUT_F){
                  IO_DruckeFlex("%*\\FCRewrite{%d%s}{%P}{%E \\wedge %C}%%\n", 
                                2*ct8_level, RE_AbsNr(gleichung), 
                                TP_RichtungAusgezeichnet(gleichung) ? "" :"'",
                                l, l1, e1, c);
                }
              }
  ...
  CLOSETRACE (("ct8_CRew -> %b\n", handled));
  return handled;
}

/* Format von ct8_handleNode
 * static 
...
static BOOLEAN ct8_handleNode (LiteralT l, ContextT c, EConsT e, HintT h)
{
  ...
  OPENTRACE (("ct8_handleNode: %t %C %E\n", l, c, e));
  {
    unsigned int oldlevel = ct8_level;
    ct8_node_counter++;
    if (TEX_OUTPUT) {
      IO_DruckeFlex("%*\\Node{%t}{%t}{%E%C}{%%\n", 
                    2*ct8_level, TO_LitLHS(l), TO_LitRHS(l), e, c);
    }
    ct8_level++;
    if (LIMIT_DEPTH && (ct8_level > DEPTH_LIMIT)){
      /* to catch endless recursion */
      our_error("ct8_handleNode: nesting to deep");
    }
    else if ((PA_maxNodeCount() != 0) && 
             (ct8_node_counter >= PA_maxNodeCount())){
      if (TEX_OUTPUT) IO_DruckeFlex("%*\\ToManyNodes%%\n", 2*ct8_level);
      closed = FALSE;
    }
    else if (ct8_doLeaf(h) && ct8_LeafTests(l,c,e,h,&closed)){
    }
    else if (ct8_doInst(h)  && ct8_Inst(l,c,e,h,&closed)){
    }
    else if (ct8_doSplit(h) && ct8_Split(l,c,e,h,&closed)){
    }
    else if (ct8_doRewr(h)  && ct8_Rewrite(l,c,e,h,&closed)){
    }
    else if (ct8_CRew(l,c,e,h,&closed)){
    }
    else {
      if (TEX_OUTPUT) IO_DruckeFlex("%*\\GiveUp%%\n", 2*ct8_level);
      closed = FALSE; /* Machen wir's nochmals explizit */
    }
    while (ct8_level > oldlevel){
      ct8_level--;
      if (TEX_OUTPUT) IO_DruckeFlex("%*}%%\n", 2*ct8_level);
    }
  }
  CLOSETRACE (("ct8_handleNode: -> %b\n", closed));
  return closed;
}
...
BOOLEAN CO_GroundJoinable8(TermT s, TermT t)
{
  BOOLEAN res = FALSE;
  {
    ...
    tt = TO_Termkopie(t);
    TO_mkLit(l,ss,tt);
    ct8_Durchnumerieren(l);
    c = CO_newContext(CO_allocBindings());
    ct8_tree_counter++;
    ct8_node_counter = 0;
    if(TEX_OUTPUT) IO_DruckeFlex("\\ctree{%d}{%%\n", ct8_tree_counter);
    ct8_level++;
    res = ct8_handleNode(l,c,NULL,ct8_mkHint(CrewHint,lambdaPos,0));
    if(0)IO_DruckeFlex("#%l %b\n", ct8_node_counter, res);
